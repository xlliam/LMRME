##################################################################################
#                                                                                #
#   R Code to analyze dietary data by usging CK_Laplace method                   #
...
##################################################################################
setwd("Your Path to Load the Data")
...
cV1 <- function(start, lambda, x_con, y, B,n_o)
{
	...
	d_st_1 <- array(0,B)
    beta_11 <- bobyqa(start,fn=FFT_AP,x_cont=x_con,y_o=y, input=input, Lambda=h,beta=beta, mconst=mconst)$par
	for (i in 1:B)
	{
...
cV2 <- function(start, lambda, x_con, y, B, n_o)
{
	...
	{
	   	w_st <- x_con + rlaplace(n_o,0,0.3390/sqrt(2))
	    w_st2 <- w_st + rlaplace(n_o,0,0.3390/sqrt(2))
	    beta_2[i,] <- bobyqa(start,fn=FFT_AP,x_cont=w_st,y_o=y, input=input, Lambda=h,beta=beta, mconst=mconst)$par
...
##############################################################################
# Part III: Run CK_Laplace method
##############################################################################
